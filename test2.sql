...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `list`  AS  select `c`.`name` AS `cashier`,`p`.`name` AS `product`,`cat`.`name` AS `category`,`p`.`price` AS `price` from ((`cashier` `c` join `product` `p`) join `category` `cat`) where ((`p`.`id_cashier` = `c`.`id`) and (`p`.`id_category` = `cat`.`id`)) ;
